...
def create_plot(coords: list[list[float]], db: Session = Depends(get_db)):
    ...
    buffer_sql = text("""
        ...
        WHERE id = :plot_id
    """)
    db.execute(buffer_sql, {"plot_id": plot.id})

    detect_inside_sql = text("""
    ...
    """)
    db.execute(detect_inside_sql, {"plot_id": plot.id})

    detect_buffer_sql = text("""
        INSERT INTO detected_features (plot_id, feature_type, location, geom)
    ...
    """)
    db.execute(detect_buffer_sql, {"plot_id": plot.id})
...
def get_plot_report(plot_id: int, db: Session = Depends(get_db)):
    ...
    rows = db.execute(feature_sql, {"plot_id": plot_id}).fetchall()

    inside = {}
    buffer = {}

    for r in rows:
        if r.location == "inside":
            inside[r.feature_type] = r.count
        else:
            buffer[r.feature_type] = r.count
...
def orthophoto_preview(
    ...
    # Reuse defaults or later fetch from DB
    render_orthophoto_png(
        db=db,
        plot_id=plot_id,
