...
def reserve_or_allocate(db: Session, *, plot: EstatePlot, customer: EstateCustomer, actor: EstatePrincipal, allocate: bool, expires_at: datetime | None = None, agreed_price=None, payment_plan: str | None = None, next_payment_due_at: datetime | None = None, notes: str | None = None) -> EstateAllocation:
    ...
    if allocate and active:
        ...
        active.next_payment_due_at = None
        ...
        return active
    reservation_expiry = None if allocate else (expires_at or now + timedelta(hours=72))
    allocation = EstateAllocation(organization_id=customer.organization_id, estate_id=plot.estate_id, plot_id=plot.id, customer_id=customer.id, status="allocated" if allocate else "reserved", reservation_date=None if allocate else now, reservation_expires_at=reservation_expiry, allocation_date=now if allocate else None, agreed_price=agreed_price, payment_plan=payment_plan, next_payment_due_at=None if allocate else next_payment_due_at, notes=notes, created_by_subject_type=actor.subject_type, created_by_subject_id=actor.subject_id, share_token=uuid.uuid4().hex)
