...
def render_green_sponsor_certificate_pdf(
    ...
    # Decorative background circles/watermarks
    c.saveState()
    c.setFillColor(HexColor("#f1f7f2"))
    # Top-right background watermark
    c.circle(page_w - 50, page_h - 50, 180, stroke=0, fill=1)
    c.setFillColor(HexColor("#edf4ee"))
    c.circle(page_w - 50, page_h - 50, 100, stroke=0, fill=1)
    # Bottom-left background watermark
    c.setFillColor(HexColor("#f1f7f2"))
    c.circle(50, 50, 150, stroke=0, fill=1)
    c.restoreState()
    ...
    # Outer gold border
    c.saveState()
    c.setStrokeColor(HexColor("#c5a059")) # Rich Gold
    c.setLineWidth(1.8)
    ...
    # Accent Gold Circles at the corners
    c.setFillColor(HexColor("#c5a059"))
    corners = [
        (margin_inner, margin_inner),
        (page_w - margin_inner, margin_inner),
        (margin_inner, page_h - margin_inner),
        (page_w - margin_inner, page_h - margin_inner)
    ]
    ...
    # --- HEADER SECTION ---
    # Centered Organization Label / Brand
    c.saveState()
    header_text = "LANDCHECK GREEN RESTORATION REGISTRY"
    char_space = 1.5
    normal_w = c.stringWidth(header_text, "Helvetica-Bold", 10)
    total_w = normal_w + (len(header_text) - 1) * char_space
    tx = c.beginText(page_w / 2 - total_w / 2, 532)
    tx.setFont("Helvetica-Bold", 10)
    tx.setCharSpace(char_space)
    tx.setFillColor(HexColor("#083e20"))
    tx.textOut(header_text)
    c.drawText(tx)
    c.restoreState()

    # Decorative small gold line under header
    c.setStrokeColor(HexColor("#c5a059"))
    c.setLineWidth(0.75)
    c.line(page_w / 2 - 120, 523, page_w / 2 + 120, 523)
    c.circle(page_w / 2, 523, 2.5, stroke=0, fill=1)

    # --- TITLE ---
    c.setFillColor(HexColor("#083e20"))
    c.setFont("Times-Bold", 25)
    c.drawCentredString(page_w / 2, 485, "CERTIFICATE OF TREE SPONSORSHIP")

    # Presentation text
    c.setFillColor(HexColor("#53705f"))
    c.setFont("Times-Italic", 11)
    c.drawCentredString(page_w / 2, 458, "This document certifies that a tree has been successfully sponsored and planted by")

    # --- SPONSOR NAME ---
    c.setFillColor(HexColor("#083e20"))
    c.setFont("Times-Bold", 28)
    c.drawCentredString(page_w / 2, 418, sponsor_name)
    ...
    c.setStrokeColor(HexColor("#c5a059"))
    c.setLineWidth(0.75)
    c.line(page_w / 2 - 180, curr_y, page_w / 2 + 180, curr_y)
    # Gold diamond accent
    p = c.beginPath()
    p.moveTo(page_w / 2 - 5, curr_y)
    p.lineTo(page_w / 2, curr_y + 3)
    p.lineTo(page_w / 2 + 5, curr_y)
    p.lineTo(page_w / 2, curr_y - 3)
    p.close()
    c.setFillColor(HexColor("#c5a059"))
    ...
    c.drawCentredString(page_w / 2, curr_y - 22, "under the LandCheck Green Restoration Initiative, contributing to environmental restoration and carbon sequestration.")

    # --- MIDDLE CARDS ROW (y from 145 to 330) ---
    middle_y = 145
    middle_h = 185

    # Card 1: Verified Tree Record (Left side)
    card1_x = 45
    card1_w = 345
    _draw_rounded_box(c, card1_x, middle_y, card1_w, middle_h, 12, fill_color=HexColor("#ffffff"), stroke_color=HexColor("#dcdfdc"))
    
    # Title banner
    c.setFillColor(HexColor("#f4faf5"))
    c.roundRect(card1_x + 10, middle_y + middle_h - 26, card1_w - 20, 18, 4, stroke=0, fill=1)
    c.setFillColor(HexColor("#083e20"))
    c.setFont("Helvetica-Bold", 8.5)
    c.drawString(card1_x + 18, middle_y + middle_h - 21, "VERIFIED TREE RECORD")

    # Tree Designation
    c.setFillColor(HexColor("#083e20"))
    c.setFont("Helvetica-Bold", 14)
    c.drawString(card1_x + 16, middle_y + middle_h - 48, tree_label)

    # Details grid inside Card 1
    grid_y = middle_y + middle_h - 58
    labels = [
        ("Project:", project_name[:42]),
        ("Species:", species_label),
        ("Location:", location_text[:40]),
        ("Planting Date:", planting_date),
        ("Issued Date:", datetime.utcnow().strftime("%Y-%m-%d %H:%M UTC"))
    ]
    
    curr_grid_y = grid_y - 12
    for label, val in labels:
        c.setFillColor(HexColor("#53705f"))
        c.setFont("Helvetica-Bold", 8.5)
        c.drawString(card1_x + 16, curr_grid_y, label)
        c.setFillColor(HexColor("#163826"))
        c.setFont("Helvetica", 8.5)
        c.drawString(card1_x + 96, curr_grid_y, val)
        curr_grid_y -= 17


    # Card 2: Photo Evidence (Center-Right)
    card2_x = 405
    card2_w = 175
    _draw_rounded_box(c, card2_x, middle_y, card2_w, middle_h, 12, fill_color=HexColor("#ffffff"), stroke_color=HexColor("#dcdfdc"))
    ...
    photo_x = card2_x + 10
    photo_y = middle_y + 40
    _draw_rounded_box(c, photo_x, photo_y, photo_size_w, photo_size_h, 4, fill_color=HexColor("#f8faf8"), stroke_color=HexColor("#e3ebe4"))
    ...
    if primary_photo:
        ...
        if reader is not None:
            try:
                c.drawImage(reader, photo_x + 4, photo_y + 4, photo_size_w - 8, photo_size_h - 8, preserveAspectRatio=True, anchor="c")
            except Exception:
                pass
        else:
            c.saveState()
            ...
            c.drawCentredString(photo_x + photo_size_w/2, photo_y + photo_size_h/2 - 3, "Verified Tree Photo")
            c.restoreState()
    else:
        c.saveState()
        c.setFillColor(HexColor("#53705f"))
        c.setFont("Helvetica-Bold", 9)
        c.drawCentredString(photo_x + photo_size_w/2, photo_y + photo_size_h/2 - 3, "Photo Pending Verification")
        c.restoreState()

    # Polaroid Label
    c.setFillColor(HexColor("#083e20"))
    c.setFont("Helvetica-Bold", 8.5)
    c.drawCentredString(card2_x + card2_w/2, middle_y + 18, "VERIFIED FIELD PHOTO")


    # Card 3: Dedication & Verification (Right side)
    card3_x = 595
    card3_w = 202
    _draw_rounded_box(c, card3_x, middle_y, card3_w, middle_h, 12, fill_color=HexColor("#ffffff"), stroke_color=HexColor("#dcdfdc"))
    
    # Title
    c.setFillColor(HexColor("#f4faf5"))
    c.roundRect(card3_x + 10, middle_y + middle_h - 26, card3_w - 20, 18, 4, stroke=0, fill=1)
    c.setFillColor(HexColor("#083e20"))
    c.setFont("Helvetica-Bold", 8.5)
    c.drawString(card3_x + 18, middle_y + middle_h - 21, "DEDICATION & REGISTRY")

    # Dedication Info
    dedication_y = middle_y + middle_h - 38
    has_dedication = bool(dedication_type or dedication_name or dedication_message)
    if has_dedication:
        c.setFillColor(HexColor("#b8860b"))
        c.setFont("Helvetica-Bold", 9)
        ded_title = dedication_type.replace("_", " ").title() if dedication_type else "Dedication"
        c.drawString(card3_x + 16, dedication_y - 8, f"{ded_title}: {dedication_name[:24]}")
    ...
    # QR Code at the bottom of Card 3
    if verification_url:
        qr_size = 65
        qr_x = card3_x + (card3_w - qr_size) / 2
        qr_y = middle_y + 12
        _draw_qr_code(c, verification_url, qr_x, qr_y, qr_size)
        c.setFillColor(HexColor("#53705f"))
        c.setFont("Helvetica", 6.8)
        c.drawCentredString(card3_x + card3_w/2, middle_y + 6, "Scan to verify live timeline")


    # --- BOTTOM CLIMATE METRICS (y from 56 to 126) ---
    metrics_y = 56
    metrics_h = 70
    
    # Calculate widths
    total_metrics_w = page_w - 90 # 751.89
    metric_card_w = 238
    gap = 18
    
    metric_x1 = 45
    metric_x2 = metric_x1 + metric_card_w + gap
    metric_x3 = metric_x2 + metric_card_w + gap
    
    # Render 3 Metric Cards
    def draw_climate_metric_card(x, val, label):
        _draw_rounded_box(c, x, metrics_y, metric_card_w, metrics_h, 10, fill_color=HexColor("#f4faf5"), stroke_color=HexColor("#cce2d1"))
        
        c.setFillColor(HexColor("#083e20"))
        c.setFont("Helvetica-Bold", 16)
        c.drawString(x + 14, metrics_y + metrics_h - 26, val)
        
        c.setFillColor(HexColor("#4f6c59"))
        c.setFont("Helvetica-Bold", 8.5)
        c.drawString(x + 14, metrics_y + 14, label)
...
def _draw_wrapped_text(
    ...
    *,
    ...
    cursor = y
    for line in lines:
        c.drawString(x, cursor, str(line))
        cursor -= line_height
    return cursor
...
def render_green_sustainability_disclosure_pdf(
    output_path: str,
    project: dict,
    summary: dict,
    *,
    ...
    """CSR-only export. Repackages the same verified data already used in the CSR Programme
    Impact Report into IFRS S2.29 / GRI 304-305 labeled line items, plus an SRG1-mapped
    Internal Control over Sustainability Reporting (ICSR) section - the shape a client's
    sustainability/finance team needs to lift figures into their own IFRS Sustainability
    Disclosure Standards filing. This document is evidence supplied TO the reporting entity;
    it deliberately does not itself make a Statement of Compliance under IFRS S1 - only the
    reporting entity can do that, per FRC Sustainability Reporting Guideline 1 (SRG1) SS2 and 15.
    """
    c = canvas.Canvas(output_path, pagesize=A4)
    width, height = A4
    csr_config = project.get("csr_config") or {}
    ...
    ink_forest = HexColor("#123629")
    ink_gold = HexColor("#b38a44")
    ink_paper = HexColor("#fbf8f1")
    ink_panel = HexColor("#fffdf8")
    ink_panel_alt = HexColor("#f5f0e5")
    ink_border = HexColor("#d7cdbb")
    ...
    def _draw_footer(page_no: int):
        ...
        c.drawString(34, 22, "Powered by LandCheck | Sustainability Disclosure Data Export - CSR programmes only")
        c.drawRightString(width - 34, 22, f"Page {page_no}")

    def _draw_box(x, y, w, h, title, lines, *, fill=ink_panel, title_color=None):
        _draw_rounded_box(c, x, y, w, h, 6, fill_color=fill, stroke_color=ink_border)
        c.setFillColor(title_color or ink_forest)
        ...
        cursor = y + h - 39
        max_lines = max(int((h - 46) / 10.2), 1)
        for line in lines:
            if cursor < y + 14:
                break
            cursor = _draw_wrapped_text(
                c, line, x + 12, cursor, w - 24,
                line_height=10.2, font_name="Times-Roman", font_size=8.2,
                color=ink_text, max_lines=max_lines,
            )
            cursor -= 2

    def _draw_metric(x, y, w, h, value, label):
        _draw_rounded_box(c, x, y, w, h, 6, fill_color=ink_panel, stroke_color=ink_border)
        c.setFillColor(ink_muted)
        c.setFont("Times-Roman", 7.0)
        c.drawString(x + 12, y + h - 16, label.upper()[:32])
        c.setFillColor(ink_forest)
    ...
    alive_trees = int(summary.get("alive_trees", 0) or 0)
    survival_pct = round((float(alive_trees) / float(total_trees)) * 100.0, 1) if total_trees > 0 else 0.0
    current_co2_t = summary.get("current_co2_tonnes", 0) or 0
    annual_co2_t = summary.get("annual_co2_tonnes", 0) or 0
    lifetime_co2_t = summary.get("projected_lifetime_co2_tonnes", 0) or 0
    area_ha = summary.get("total_existing_area_ha", 0) or 0
    missing_age = int(summary.get("trees_missing_age_data", 0) or 0)
    fallback_age = int(summary.get("trees_with_fallback_age", 0) or 0)
    review_pct = summary.get("review_coverage_pct", 0) or 0
    map_pct = summary.get("map_coverage_pct", 0) or 0
    custodian_count = int(summary.get("custodian_count", 0) or 0)
    maintenance_total = int(summary.get("maintenance_total", 0) or 0)
    maintenance_done = int(summary.get("maintenance_done", 0) or 0)
    top_species = summary.get("top_species") or []
    material_topics = [str(item).strip() for item in (csr_report_profile.get("material_topics") or []) if str(item).strip()]
    ...
    _draw_header_bar(
        ...
        "Environmental data supporting IFRS S1/S2 & GRI-aligned sustainability disclosure",
        bar_height=82,
    )
    ...
    scope_lines = [
        ...
        f"Programme: {str(csr_report_profile.get('programme_title') or 'CSR Environmental Programme')}",
        f"Reporting cycle: {reporting_cycle}",
    ...
    # ---------------- Page 2: Climate-related metrics + biodiversity ----------------
    _draw_header_bar(
        "Climate-Related Metrics (IFRS S2.29) & Biodiversity (GRI 304-305)",
        "GHG removals, verified footprint, and species/habitat data mapped to disclosure line items",
        bar_height=68,
    )
    ...
    if top_species:
        ...
        colors = ["#1f7a3d", "#2f9e44", "#52b788", "#74c69d", "#0ea5e9", "#f97316"]
        for idx, item in enumerate(top_species[:6]):
            chart_data.append((str(item.get("species") or "Unknown")[:20], float(item.get("co2_kg") or 0.0), colors[idx % len(colors)]))
        _draw_rounded_box(c, 34, species_y, width - 68, species_h, 6, fill_color=ink_panel, stroke_color=ink_border)
        c.setFillColor(ink_forest)
        c.setFont("Times-Bold", 10.5)
        c.drawString(46, species_y + species_h - 18, "Species / Habitat Composition (GRI 304 basis)")
        c.setFont("Times-Roman", 7.6)
        c.setFillColor(ink_muted)
        c.drawString(46, species_y + species_h - 30, "Species distribution by current CO2 removal contribution (kg), used as the biodiversity composition basis.")
        _draw_bar_chart(
            c, 46, species_y + 16, width - 92, species_h - 56, chart_data,
            title="", title_font="Times-Bold", label_font="Times-Roman", value_font="Times-Roman",
            title_color=ink_forest, label_color=ink_muted, value_color=ink_text,
        )
    else:
        _draw_empty_state_box(
            34, species_y, width - 68, species_h,
            "Species / Habitat Composition (GRI 304 basis)",
            ["No eligible species-level records are in scope yet for this reporting cycle."],
            fill_color=ink_panel,
        )
    ...
    icsr_rows = [
        ("Clear ownership of data by a responsible officer", f"Every field record is attributed to a named field officer or custodian ({custodian_count} custodian(s) on record); task assignments are tracked by name."),
        ("Documented methodology", "IPCC Tier 1 defaults + Chave et al. (2014) pantropical allometric equations, applied per species with a documented growth model; see Methodology & Sources below."),
        ("Review and approval of assumptions/estimates", f"{_fmt_num(review_pct, 1)}% of records carry a completed submit-review-approve workflow with reviewer identity and timestamp."),
